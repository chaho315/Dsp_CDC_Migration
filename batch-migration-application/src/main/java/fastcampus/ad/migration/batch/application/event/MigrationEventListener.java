...
public class MigrationEventListener {
    ...
    public void handlerMigrationProgressedEvent(MigrationProgressedEvent event) {
        log.info("migration progressed event listener: {}", event.toString());
        streamBridge.send(MIGRATION_USER_OUT, MessageBuilder.createMessage(MigrationUserMessage.from(event), new MessageHeaders(Map.of("partitionKey", event.getUserId()))));
    }

    @TransactionalEventListener
    public void handlerPageMigrationEvent(PageMigrationEvent event) {
        log.info("page migration event listener: {}", event.toString());
        streamBridge.send(PAGE_MIGRATION_OUT, MessageBuilder.createMessage(PageMigrationMessage.from(event), new MessageHeaders(Map.of("partitionKey", event.userId()))));
    }
}
